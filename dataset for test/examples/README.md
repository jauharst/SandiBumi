...
| `bad_truncated.las` | the last data row is cut mid-line (RHOB missing, no trailing newline) | **Clean error** naming the file, bad line, short ASCII row and "truncated or corrupt LAS" — nothing imported, not even the 39 rows that were intact. A well short by one row is a well nobody knows is short. |
