...
| `SB-DBM-030` | Engineering - REMAINDER NARROWED 2026-08-18 | **The screen, its flag channel and the import-store SQL NULL binding SHIPPED** (strict computed-bound screen on Geolog cgg.h -1.0e30; generic store and standard projection both screen and both bind NULL for NaN; every screened count surfaces by mnemonic through LAS warnings, DlisSkip and intake notes; migration keeps a data-bearing column's full frame; two qualifying tests, six mutations killed on six distinct assertions). | What remains is ONE writer: `equations::write_versioned_rows_raw` still appends NaN to `computed_curves`/archive as a float, so measurement-absent is representable as a number in that store alone. Binding it needs the ~135 computed_curves reader sites audited for NULL tolerance first (a `row.get::<f32>` errors on NULL). Engineering only; no input owed from Jauhar. |
