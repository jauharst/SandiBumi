...
| `SB-SAT-027` | Same `lrlc.rs` / `multimin2.rs` authorization | Behaviour is `PRESENT-OK`. **Re-read against the chapter 2026-08-17 and the register is OVER-STRICT.** `12_saturation.md:1425-1431` explicitly permits the closed form: a fast path for `n = 2` **MAY** be used and **MUST be asserted equal** to the general solver - so routing every equation through one solver is NOT required. The real gap is narrower: the guard contract (seed 0.5, max 20 iterations, tolerance `|d| < 1e-5`, `sat = MAX(0, sat)` at each step) is not cross-asserted between `modules.rs` (which transcribes Geolog `CALC_SW`) and `multimin2.rs` (`solve_simandoux_root`, `sw_cond_root`). Existing `sw_sim_matches_quadratic_solution` compares module to closed form, not engine to engine; `every_model_shared_by_the_module_and_solver_engines_...` is owned by SB-CORE-006 and pins EQUATION agreement, not guard agreement. Nothing is registered for `SB-SAT-T12`/`T41`. | Write ONE cross-assertion test pinning the guard suite and the `n = 2` closed-form equality ENGINE AGAINST ENGINE. `solve_simandoux_root` and `sw_cond_root` are private `fn` in `multimin2.rs`; exposing them as `pub(crate)` is the only production change needed and falls inside DEC-048 narrow authorization. No re-routing, no solver merge. |
