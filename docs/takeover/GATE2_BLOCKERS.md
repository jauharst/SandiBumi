...
|---|---|---|---|
| `SB-CORE-044` | Source and legal | Endpoint-library arm CLOSED by DEC-078 (2026-08-19, per-value custody shipped); theme arm CLOSED by DEC-074. Remaining: the chart-payload arm follows SB-PLT-024 (re-derivations + the Gate 5 eleven-id decision per DEC-080), and counsel dispositions (CLAIM-012/013, dependency attention items) stay open first-sale items. | Execute SB-PLT-024; obtain counsel dispositions; then add the fail-closed release-inventory proof over the settled shipped set. |
| `SB-PLT-024` | Legal route ruled; execution far advanced | DEC-078 rules the route; DEC-079 rules the two fidelity questions (por22_fo ships the published RHG-1980 algorithm, not the tracing; lith3/4 ship Gardner-Dumanoir physics); DEC-080 reclassifies por20_ta/por20_fo as tool-response (CNL-coupled neutron axis), so the derivable class holds eight definitions and the retained register eleven (src/ui/chartOverlayPolicy.ts, pinned disjoint from the derived set by test). SIX of the eight are derived and proven (por22_ta, por22_fo, lith3, lith4, lith6_mid, lith2_thk — the last from the chart page's own printed boundary values, executed 2026-08-19 after Quirein SPE 11143 and Hassan-Hossin-Combaz 1976 were obtained and read in full). | Remaining derivations: lith1_pek/lith1_pethk only. Increment-6 finding: NEITHER obtained paper prints numeric per-mineral K/Th values (Quirein prints formulas and the model — thorium is a trace element no formula yields; Hassan prints the data only as figure graphics), so the lith1 mineral positions await Edmundson & Raymer 1979, "Radioactive Logging Parameters for Common Minerals", The Log Analyst 20(5) pp 38–47 (Quirein's own ref 2 — the numeric table) — Jauhar to obtain; else those two defs need their own class ruling. At Gate 5 Jauhar rules keep-licensed or delete for the eleven; counsel closes CLAIM-013 on whatever ships. |
