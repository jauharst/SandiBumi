...
| `SB-SAT-025` | Engineering - DEC-048 RULED | Half is **ready**: `sw_arch` has `SWT_ARCH` but no `SWE_ARCH`, and that fix is in allowed `modules.rs`. The other half - LRLC emitting clamped values only (`lrlc.rs:183`, `:365`) - is prohibited. Held atomic because the MUST covers *every* method. A clipped-only curve cannot distinguish *the rock is wet* from *the model went out of range*. | **Both files were authorized by DEC-048 on 2026-08-16 and this demand was stale; DEC-062 now opens the whole tree.** Add `SWE_ARCH` and the LRLC twins, and pin that an out-of-range sample shows the clipped curve at its bound AND the diagnostic beyond it. |
| `SB-SAT-026` | Engineering - DEC-048 RULED | As-built said *no method-flag curve exists*; one does - `SW_METHOD`, on 3 of 7 saturation modules. The gap is **coverage**: `sw_rtc`, `sw_imts`, `multimin` and `sw_height` lack it, and all four are in prohibited files. The naming clause (no bare `SW`/`SXO`) is already true and only needs its enforcement test, which needs no authorization. | **All three files were authorized by DEC-048 on 2026-08-16 and this demand was stale; DEC-062 now opens the whole tree.** Make the remaining four modules emit `SW_METHOD`, then pin both clauses universally, so a future saturation module cannot ship without a method flag. |
