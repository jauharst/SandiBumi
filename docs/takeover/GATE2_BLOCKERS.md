...
| `SB-DBM-030` | Engineering - DEC-027 RULED | One contract requires a queryable `REQUIRED_UNSET` row while another says absence-of-row; the requested Geolog export bound is also explicitly non-adopted. | Choose the named-row contract or correct it, and authorize only a cited production magnitude or a clearly test-only verification constant. |
...
| `SB-DBM-032` | Engineering - DEC-028 RULED | The closed installer contract refuses a missing ordinal while the DBM test expects a one-handle legacy row to load with warning. | Define semantic-only and ordinal-only policy without reinterpreting an ordinal or weakening disagreement refusal. Recommended: refuse both one-handle forms. |
