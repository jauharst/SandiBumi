...
| `SB-POR-002` | One method ruling (DEC-063) | **DEC-038 is implemented as of 2026-08-18**: SSC/SSPW are typed WORKFLOWS (registry change only - outputs pinned numerically unchanged, `ssc.rs` untouched), and a registered METHOD is now refused unless it declares its unlimited/limited pair while a WORKFLOW is exempt - both sides pinned, five mutations killed. What remains is sonic: the bare unlimited pair SB-POR-002 demands and SB-POR-009's every-sample `PHIT >= PHIE` are jointly unsatisfiable for the current transform at in-range endpoints (`DT_SH < DT_MA` turns the shale subtraction into an addition; the SB-POR-009 test constructs the witness). `phi_son` is the one NAMED pending exemption, tied to DEC-063. | Jauhar answers DEC-063: **(a)** the ordering guarantee applies to the LIMITED pair - the inverted unlimited sonic pair IS the diagnostic of a bad `DT_SH`/`DT_MA` pick (consistent with DEC-038's own reasoning) - or **(b)** the sonic twin waits for the SB-POR-013/014/015 rebuild. If (a), engineering finishes the row with no further input. |
