...
|---|---|---|---|
| `SB-CORE-044` | Source and legal | Endpoint-library arm CLOSED by DEC-078 (2026-08-19, per-value custody shipped); theme arm CLOSED by DEC-074. Remaining: the chart-payload arm follows SB-PLT-024 (re-derivations COMPLETE per DEC-081; the Gate 5 thirteen-id decision), and counsel dispositions (CLAIM-012/013, dependency attention items) stay open first-sale items. | Execute SB-PLT-024; obtain counsel dispositions; then add the fail-closed release-inventory proof over the settled shipped set. |
| `SB-PLT-024` | Legal route ruled; derivation program COMPLETE | DEC-078 rules the route; DEC-079 the two fidelity questions; DEC-080 reclassifies Por-20 (CNL-coupled); DEC-081 reclassifies Lith-1 — its mineral areas are vendor synthesis whose entire cited chain was obtained and READ on 2026-08-19 (Edmundson & Raymer 1979 prints Pe but no thorium; Quirein 1982 formulas only; Hassan 1976 and Hassan & Hossin 1975 — the root source, read from the Gallica scan of C.R. t.280 pp. 533–535 — figure graphics only; Serra 1981 internal). The derivable class closed COMPLETE at six of six executed (por22_ta, por22_fo, lith3, lith4, lith6_mid, lith2_thk); the retained register holds thirteen ids (chartOverlayPolicy.ts, pinned disjoint from the derived set by test). | All engineering on this row is done. Remaining: Jauhar's Gate 5 word on the thirteen retained ids (keep-licensed or the single named delete); counsel closes CLAIM-013 on whatever ships. |
