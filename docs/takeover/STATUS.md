...
- Gate 2 requirement progress: `26 / 222 handled — 19 DONE, 7 BLOCKED, 196 remaining`
- Baseline foundation: `COMPLETE`
- Active increment: `G2-T03 / SB-DBM-006 - DONE; calculation, plotting identity and schema-v2 ancestry now share one staged curve resolver that records the exact chosen and rejected identities, set versions and decision rule`
...
- Automated gate: `SB-DBM-T08 pins both sides of a three-GR Final reflag, exact chosen and rejected curve/set/version identities, the rule, numeric source and fail-closed incomplete records; a two-sided RAW-versus-attached regression preserves RAW absolute priority and the fallback when RAW is absent; the full repository gate is green at 991 passed / 0 failed / 36 ignored and the owned Rust warning inventory remains 55`
- Pilot field evidence: `OPEN`
- Open blockers: `196 Gate 2 rows remain unhandled; SB-CORE-003 is BLOCKED on the complete cited pilot-method inventory; SB-CORE-005 is BLOCKED on exact per-value endpoint custody plus CLAIM-012 counsel disposition; SB-CORE-007 is BLOCKED on T23 semantics for ABSENT-default producers and intentional working-curve replacement/method flags; SB-CORE-015 is BLOCKED on the absent RP66 writer source and DLIS export path required by T15; SB-CORE-044 is BLOCKED on the chart-payload, endpoint-library, branded-theme and dependency counsel/source routes named in its evidence row; SB-DBM-002 is BLOCKED on DEC-021 selecting the exact build-derived module identity source; SB-DBM-005 is BLOCKED on the complete registered-module derivation-source map; the approved 222-row scope remains immutable; 20 approved rows are explicitly owned by Gate 3 or Gate 4`
- Next increment: `After committing SB-DBM-006, adjudicate SB-DBM-007's named missing-provenance states without confusing them with f32 sample missingness.`
...
| G2 — Silent-wrongness closure | IN PROGRESS | exact program: 222 owned here / 20 later-gate-only; current live classes 34 implement-or-refuse / 110 remediate / 18 prove / 60 retain; initial routing was 36 / 124 / 19 / 43; ten serial tranches; final audit remains open |
