...
### SB-CUT-001 - One discretisation rule, parameterized, defaulting to CENTRED

- **Specified contract:** one shared thickness discretisation rule across every summation consumer, with a discretisation-model parameter defaulting to CENTRED; the shipped FORWARD interval stays available.
- **Current implementation (2026-08-18):** DONE. The triplication was already gone (one `sample_incl_thickness`, three consumers). **DEC-071** ruled the default: Jauhar chose CENTRED after the difference was explained in thickness terms, accepting that every existing net/NTG number moves at pay and zone edges (half a sample step per edge). `workflow::DiscretisationModel` {Centred default, Forward} carries ONE vocabulary - the serde wire form is the record token (`CENTRED` / `TOPS`); `workflow::sample_slab` is the single slab derivation; pay summary, cutoff sweep and `montecarlo::zone_metrics` all take the request field, `#[serde(default)]` so a pre-ruling request gets the ruled default. Every thickness-bearing record (and the report heading) names the model that actually produced it. Existing numeric fixtures declare Forward explicitly - their hand-derived expectations pin the forward arithmetic, still selectable to reproduce a legacy run.
- **Qualifying test:** `the_discretisation_model_defaults_to_centred_and_forward_reproduces_the_shipped_numbers` - CENTRED default on the enum and over the wire; slab geometry both models; hand-derived distinct numbers on a data-edge zone (FORWARD 1.0 vs CENTRED 1.5); per-model record tokens; and Monte Carlo net agreeing with the deterministic pay summary under BOTH models (the DEC-071-noted contract). Five mutations killed on distinct assertions. Test class `CORRECTNESS`.
...
- **Current implementation - verified in code.** `PaySummaryRow` (`paysummary.rs`) carries `well_id`, `well_name`, `zone`, `flag`, `top`, `bottom` and the summation numbers, and **no discretisation model or sample interval**. Monte Carlo is in scope too and equally silent: `montecarlo.rs:272-273` emits `net` and `ntg` percentile bundles per zone with no model recorded.
...
- **Current implementation / as-built:** `PARTITION_TOLERANCE = 1e-7` and `reconcile_partition` in `paysummary.rs`, with two structured types - `ReconciledPartition {net, not_net, unknown, absorbed}` and `PartitionResidual {gross, net, not_net, unknown, residual, relative, tolerance}` whose `Display` names every one of them. `run_pay_summary` calls it on every zone-by-flag row and propagates the refusal prefixed with the well, zone and flag. `PaySummaryRow` gains `residual_absorbed`. PRESENT-OK.
...
- **Current implementation / as-built:** `AverageWeighting {Thickness, Porosity}`, `AVERAGED_SLOTS`, `default_weighting`, `weighting_for` and a `WeightedMean` accumulator in `paysummary.rs`; `PaySummaryRequest.weighting` is a `BTreeMap` keyed by SLOT, persisted with the rest of the run configuration in `log_sets.params_json`. `run_pay_summary` keeps one accumulator per averaged slot carrying whichever weighting the run declared, replacing the four hand-rolled sums that hard-wired the φ-weighted form to the saturation slot. PRESENT-OK.
...
- **Current implementation / as-built:** `SummationFrame {Md, Tvd, Tvdss, Tst}` and `MD_WEIGHTS_SOURCE` in `paysummary.rs`; `PaySummaryRequest.frame` (serde default MD, persisted with the run configuration in `log_sets.params_json`) and `PaySummaryRow.frame` / `.weights_source`. PRESENT-OK.
...
- **Release disposition and risk:** PILOT-BLOCKER; SILENT-WRONGNESS.
- **Automated evidence:** `no_cutoff_ships_a_value_an_unapplied_one_is_reported_unfiltered_and_an_enabled_blank_one_refuses` (`src-tauri/src/paysummary.rs`). CORRECTNESS. Five arms: a source scan proving neither UI file seeds any of the forbidden vendor numbers; a fully specified run where the cut-off bites; an unfiltered run where it does not AND the row reports it; **rock that fails every vendor default counting in full when nothing is set**; and an enabled-but-blank cut-off refusing by name.
...
- **Current implementation / as-built:** `BoundOperator {Inclusive, Exclusive}`, `CutoffBound`, `CutoffRange` with its `contains`, `CutoffSense` and the `CutoffSpec` wire type in `paysummary.rs`. Six request shapes carry `Option<CutoffSpec>`; `classify_sample`, `compute_sweep` and `montecarlo::zone_metrics` all compare through `CutoffRange::contains`, which is now the only place in the product where a cut-off comparison is made. PRESENT-OK.
...
- **Current implementation / as-built:** `CutoffUse {sand, reservoir, pay}`, `default_cutoff_use`, `cutoff_use_for` and the run-scoped `TierCutoffs` in `paysummary.rs`; `PaySummaryRequest.cutoff_use` carries the declaration and is persisted with the rest of the run's configuration. `classify_sample` is now tier-driven: each tier applies exactly the cut-offs declared for it. PRESENT-OK.
...
- **Release disposition and risk:** PILOT-BLOCKER; FIELD-EVIDENCE.
- **Automated evidence:** `a_wet_but_porous_clean_sand_is_reservoir_and_not_pay_because_saturation_enters_at_the_pay_tier` (`src-tauri/src/paysummary.rs`). CORRECTNESS. Five arms over a purpose-built fixture - clean at VSH 0.10, porous at PHIE 0.30 and WET at SWE 0.80, which is exactly the rock the row protects. The wet sand books as reservoir IN FULL and as pay NOT AT ALL; the reservoir tier does not move when the saturation cut-off's value moves, while pay does, which distinguishes *does not apply* from *applies but happens not to bite on this fixture*; the reservoir tier IS clay-driven, pinned from the positive side so that a tier applying NOTHING cannot pass; an explicit declaration still reaches the reservoir tier, because the requirement forbids it BY DEFAULT and not at all; and the default is readable off the configuration rather than inferred from a result.
...
- **Current implementation / as-built:** `ClampStage {Accumulate, FlagTest, Present}`, `BoundedQuantity {VolumeFraction, Permeability, Unbounded}` with `bounds()` and `is_out_of_range()`, and the single `stage_value` that expresses the policy, all in `paysummary.rs`; `PaySummaryRow.out_of_range` carries the condition. All three stages have real call sites. PRESENT-OK.
- **Why `accumulate` must not clamp, in the chapter's own arithmetic:** for a truly wet interval the unclamped hydrocarbon contribution `phi*(1-Sw)` has expectation **zero** under symmetric noise, while the clamped `phi*max(0, 1-Sw)` has expectation `phi*sigma/sqrt(2*pi)` = **`0.3989*phi*sigma > 0`**. Clamping does not relocate a tail - it moves the **mean**, always toward more hydrocarbon, by an amount **independent of iteration count**. An argument that is correct for one deterministic evaluation is a bias in expectation over an ensemble.
- **Where each stage now lives.** `Accumulate` at the weighted-average summation, named at the site so a future edit has to argue with the name. `FlagTest` at the cut-off comparison, clamping to the **quantity's** bounds. `Present` at the emitted averages and at HPV. **No number moves**: the flag-test clamp is inert for an in-range sample, which is every ordinary one, and all 1,043 backend tests passed unchanged.
- **Bounds attach to the QUANTITY, never to a curve-type string.** That is the specific failure that makes IP's clipping worse than Techlog's unconditional clamp: IP clips by *declared curve type*, so mis-typing a curve silently changes its numerics and the change is **invisible in the data**. A quantity cannot be mis-typed by a label, because it is not a label.
- **`Unbounded` has a real site, not a ceremonial one.** HPV is a volume-thickness that routinely exceeds 1, so it is presented as an unbounded quantity - which is the clause *"an unbounded quantity MUST NOT be clamped to [0,1]"* stated where a careless clamp would destroy the number rather than merely round it. Permeability is bounded below at zero and **open above**, so a real 4,000 mD survives.
- **An out-of-range average is EMITTED, not corrected.** `present_average` clamps an in-range value (inert) and deliberately lets an out-of-range one through beside its flag. A corrected average is a number nobody derived, and the condition that produced it is exactly what a reviewer needs to see. The flag rides a typed sibling for SB-CUT-029's reason.
- **Release disposition and risk:** PILOT-BLOCKER; SILENT-WRONGNESS.
- **Automated evidence:** `accumulate_never_clamps_while_flag_test_and_present_clamp_to_the_quantitys_own_bounds` (`src-tauri/src/paysummary.rs`). CORRECTNESS. Seven arms: the stage rule at out-of-bounds values on both sides for every quantity; the quantified bias reproduced, with a symmetric spread accumulating to zero unclamped and to a strictly positive sum clamped; bounds attached to the quantity, with 4,000 mD surviving and an unbounded quantity not clamped to `[0,1]`; out-of-range detection with a NaN explicitly NOT out of range, because absent and impossible are different statements; percent-to-fraction conversion and the bound check shown to be separate operations, with the refusal for `200 pu` naming the CONVERTED 2.0 - which is what proves the check ran after the conversion rather than instead of it; an in-range well flagging nothing; and an impossible saturation of 1.40 flagged AND emitted as computed.
- **A mutation that PASSED, and the hole it found.** Replacing the computed `out_of_range` with a hard `false` did not turn the test red: the in-range control is satisfied by a flag that can never be true. The positive arm - a well whose saturation average is 1.40, asserted both FLAGGED and NOT CORRECTED - was added because of it, and the re-run probe now fires there.
- **A second thing the gate caught, and it was right.** The first implementation declared the stages and used none of them in production. The hygiene gate refused it as dead code, which is the correct verdict: a stage abstraction nothing routes through is ceremony, and the requirement is about where the boundaries ARE, not about naming them somewhere. All three now have call sites.
- **Mutation evidence:** five probes, each read for WHICH assertion fired, all five at distinct assertions. Giving `accumulate` a clamp turned the stage arm red. Giving `Unbounded` the `[0,1]` bounds - IP clipping by declared type, reproduced - turned the unbounded arm red. Capping permeability at 1 turned the 4,000 mD arm red. Correcting an out-of-range average instead of flagging it turned the positive arm red. Making a NaN read as out of range turned the absent-versus-impossible arm red.
- **The evidence register carried a CHARACTERIZATION row and it is replaced, not joined.** `flooring_phie_leaves_missing_missing` is a real test and stays in the suite; it pins the PHIE floor's missing-data behaviour, not the stage separation, and would pass unchanged with every clause of this row violated.
- **Named limit, and it is Jauhar's to rule on.** The Monte Carlo accumulator still reads the chain's **limited** curves, so the configuration in which accumulation sees genuinely unclamped data exists in the pipeline but is not what the shipped default feeds it. Section 3.8 bounds the exposure honestly and this record repeats it rather than softening it: accumulation runs over pay-flagged samples only and the flag test runs on the same clamped curves, so with a restrictive saturation cut-off the Sw clamp at 1 is **inert**. The bias is fully live in exactly two configurations - a permissive cut-off set (`swe_max = 1`, `phie_min = 0`, which is the whole-zone-average configuration and precisely what Geolog's `tp_pay_sensitivity` ships) and any future reporting of a zone average that is not pay-restricted. Re-pointing the accumulator would change every existing Monte Carlo result in those configurations, and the chapter specifies the stage contract without specifying which unlimited companion pairs with a limited curve when a chain emits several. That pairing is a method decision and it is not made here.
- **Manual evidence:** cutoffs-pay 0/23; monte-carlo 2/14. Automated only.
- **Source/parameter boundary:** no value adopted. The bounds are the physical range of a volume fraction and of a permeability; the `0.3989` is the chapter's own arithmetic, reproduced by the test rather than hard-coded as an expected number.
- **UI/IPC/provenance surface:** `PaySummaryRow.out_of_range` crosses the wire and is declared in `ipc.ts`, to be rendered as a warning beside the value and never by rewriting the value.
- **History/reachability:** the limited and unlimited companion curves and the plausibility diagnostic were integrated; the stage separation, the quantity-attached bounds and the flag are new.
- **Blocking decision / next action:** cleared, with the Monte Carlo accumulator's input left as a stated decision for Jauhar.
