...
- **Supporting tests:** `export.rs::a_feet_project_las_round_trip_preserves_depths_and_declares_ft_on_every_depth_header`, the explicitly characterized metre counterpart and LAS re-import tests exercise the export side but cannot prove the word “every.”
...
- **Atomic obligations:** each physical constant and transform has one canonical definition; deliberate duplicate implementations agree on a shared vector; every producer of the same output uses the same sourced default in a no-parameter fixture.
- **Current source:** live T19 discovery found one true finite-default conflict: `RHO_MA` is 2.645 in `vsh_dn`, `phi_den`, `phi_dn` and `condflag`, but 2.65 in `gascorr`. It also found `K` at 10 for nearest-neighbour prediction and 5 for two facies-count parameters, proving raw spelling alone is not a physical-constant identity. T20 still has the duplicated eight-method GR ladder plus SSC/SSPW gas branches; SSPW retains the obsolete 1.6 weight. T23 discovery found repeated declared output keys for `FTEMP`, dynamic `OUT_CURVE`/`OUT_FLAG` placeholders, `PERM`, `PHIE`, `PHIT`, `SWE`, `SW_METHOD`, `VOL_UWAT`, `VSAND` and `VSH`. Several are intentionally different methods, and `SW_METHOD` is categorical by SB-CORE-006.
- **Qualifying acceptance tests:** none. Two deliberately RED discovery tests enumerated T19 and T23 against the complete live registry, then were removed: committing either failure or weakening it into an allowlist would not satisfy the owned tests. T19/T20/T23 remain `MISSING`.
- **Supporting tests:** method-local fixtures validate isolated copies and cannot prove global parity. SB-CORE-004 proves an `ABSENT` required parameter has no concealed default and refuses until an interpreter supplies one; that makes T23's mandated no-parameter run impossible for such a producer under the current wording.
- **Manual evidence:** `workflow` 0/23, `equation-engine` 1/14, `sandimin` 0/28 and `verification-stewardship` 2/33 before this review entry; no manual or field scenario was checked here.
- **Git evidence:** divergent definitions are integrated at the accepted anchor. The discovery tests changed no production behavior and were removed before this evidence-only increment; the full gate is green at 977 passed / 0 failed / 36 ignored, and no universal closing commit is present.
- **Verdict:** `PRESENT-DIVERGENT`; `PILOT-BLOCKER`; `SILENT-WRONGNESS`; test class `MISSING`; commit state `INTEGRATED`; Gate 2 progress state `BLOCKED`.
- **Blocker or decision:** T23 does not say how to run a producer at its own shipped defaults when one or more required parameters deliberately ship `ABSENT`, and supplying them is expressly forbidden. It also does not distinguish a canonical method output from DEC-013's explicit user-selected working-curve replacement or from `SW_METHOD`, whose values must differ by method. Treating refusals as equal outputs, exempting an allowlist, or forcing different methods numerically equal would weaken or falsify the requirement.
- **Next action:** adjudicate those two T23 boundaries without inventing an exception: define executable behavior for ABSENT-default producers and a typed separation between canonical method outputs, explicit working aliases and categorical method identity. Then add semantic constant identities, centralize each cited constant/transform, run every genuine duplicate producer at its untouched defaults, and implement T19/T20/T23 universally.
...
- **Current source:** `equations.rs` owns a validated `CurveAncestry` record and opaque complete-set/write types. The record carries module/version, each effective input's well/curve/log-set/version/id, every parameter value and source, per-well zone scope, explicit HUMAN/AUTOMATED session actor, backend timestamp and output derivation. Module, chain, equation/Python, Monte Carlo, SandiMin, ML, reframe, core-photo, deviation, net-flag, pay-summary and computed-edit/undo paths create one complete per-well version before writing; no production caller can use the test-only raw batch helper. Missing actor/source or malformed input ancestry refuses before version allocation or current-row replacement. Curve Catalog/Inspector exposes backend-resolved records. LAS, ordinary and composite PNG/SVG/PDF, report PDF and Office paths carry the same record, and current/archive set versions retain it.
- **Qualifying acceptance tests:** `core_ancestry_tests::every_computed_curve_written_by_any_module_has_a_complete_ancestry_record` is T14: it pins complete success and missing actor/source refusal from both sides, scans production Rust for ancestry-free writers, and verifies UI/deliverable disclosure formats. `core_ancestry_tests::a_complete_ancestry_record_round_trips_through_project_save_and_load` is T15: it saves, reopens and compares the complete record exactly. Test class `CORRECTNESS`; expected structure comes from SB-CORE-010/T14/T15, not from a snapshot of current code.
- **Supporting tests:** the LAS refusal/self-read tests, log-set version/archive tests, computed-edit undo tests, plot/report/Office export tests and project Save As tests exercise their individual surfaces; T14/T15 own the cross-surface minimum and exact reopen proof.
- **Manual evidence:** `processing-history` 0/7, `database-tools` 0/2 and `verification-stewardship` 2/33 before this increment are unexercised for complete ancestry; `project-lifecycle` remains partial at 3/24. No manual or field scenario was checked here.
- **Git evidence:** the closing implementation and T14/T15 are integrated in the SB-CORE-010 Gate 2 increment; exact-tree verification is recorded in `STATUS.md` and `REVIEW.md`. Full gate: 979 passed / 0 failed / 36 ignored; Rust warning inventory remains the owned 56.
- **Verdict:** `PRESENT-OK`; `PILOT-BLOCKER` (satisfied automated data-integrity contract); `DATA-INTEGRITY`; test class `CORRECTNESS`; commit state `INTEGRATED`; Gate 2 progress state `DONE`.
- **Blocker or decision:** `DEC-020` records the approved explicit session actor and source/reference custody. There is no remaining automated SB-CORE-010 blocker; manual and field evidence remain open and are not inferred from the green gate.
- **Next action:** proceed to SB-CORE-011. During Gate 4, Jauhar inspects one approved raw-to-pay run's record in the app, delivered files and reopened Save As project. `DEC-009` continues to govern lineage beyond the SB-CORE-010 minimum.
...
- **Current source:** `param_sources.rs` owns the exact 15-topic DEC-003 pilot registry: cluster count plus the cited GR endpoints, porosity endpoints, Archie `a/m/n`, `Rw`, `Rsh`, and deterministic VSH/PHIE/SWE cutoffs. Each position carries product, value or explicit absence, context, exact source and owning chapter tier; no source row becomes a SandiBumi default. Module manifests and every pilot cutoff editor point to that shared registry. `workflow.rs`, `chain.rs` and pay-summary ancestry persist a structured snapshot of the alternatives and whether the selected numeric value matches any cited position; old ancestry remains readable because the decision field is optional and version-one compatible. Deliverable ancestry disclosures render the persisted snapshot instead of re-querying a future registry.
- **Qualifying acceptance test:** `every_pilot_parameter_with_competing_values_shows_its_sources_and_tiers_and_persists_the_interpreters_choice` is `CORRECTNESS`. Its expected inventory and values come from the cited §5 tables in the CLY/POR/SAT/CUT/MLA chapters. It pins exact and range matches against an interpreter-owned unmatched choice, every selected manifest/pay topic binding, a real persisted matching and non-matching VSH run, retained custody source, and all three pay-cutoff decisions.
- **Supporting tests:** `every_cluster_count_position_names_its_product_source_and_tier_and_keeps_explicit_absence` preserves the pre-existing cluster-source inventory as independent coverage; `the_recorded_cluster_choice_distinguishes_a_cited_range_from_an_interpreters_own_value` pins range attribution and explicit non-attribution. Neither is counted as the owned CORE-013 acceptance test.
- **Visual evidence:** a real debug Tauri window ran against an isolated configuration, WebView profile and temporary project. At 802×632, the maximized VSH editor showed GR_MA and GR_SH source prompts collapsed by default; one click expanded only GR_MA and exposed Techlog, Geolog and IP values with tier/source lines. This inspection found and fixed a CSS rule that had overridden `hidden` and opened every panel. It is automated visual evidence, not Jauhar's manual or field acceptance.
- **Manual evidence:** `machine-learning` 7/189 is partial; `verification-stewardship` 0/24 is unexercised. No manual or field scenario was marked complete.
- **Git evidence:** implementation and acceptance proof are integrated on the active Gate 2 branch pre-PR; focused source, ancestry, pay-summary and deterministic tests are green.
- **Verdict:** `PRESENT-OK`; `DONE` for the exact DEC-003 pilot inventory; `DATA-INTEGRITY`; test class `CORRECTNESS`; commit state `INTEGRATED`.
- **Blocker or decision:** none for the approved pilot scope. Post-pilot expansion to disagreements outside DEC-003 remains deferred and this closure is not a universal all-product claim.
- **Next action:** retain the registry, persistence proof and collapsed/expanded UX. Add a future topic only when its own chapter provides the cited position, source and tier; never infer a nearest vendor or turn disclosure into a default.
...
- **Qualifying acceptance tests:** `an_exported_las_reimports_with_the_same_values`, `a_feet_project_las_round_trip_preserves_depths_and_declares_ft_on_every_depth_header`, and `every_registered_writer_reads_its_output_and_a_rejected_round_trip_is_an_error` are `CORRECTNESS`, using the declared fixture values and NIST SP 811 conversion. T15 remains missing.
- **Supporting tests:** the metre round trip is explicitly `CHARACTERIZATION`; optional-package office round trips are ignored correctly but do not provide an always-green universal writer inventory.
...
- **Current source:** all `run_simple_job` call sites are registered non-cancellable. The seven current `run_job(..., true, ...)` families route to LAS ingest, equation, workflow module, Monte Carlo, ML and SandiMin workers that poll a `JobHandle` or raw shared flag. The manual workflow-chain registration shares the same flag with both Cancel surfaces. `run_job` finalizes as Cancelled only after a worker observed the set flag, and the processing panel shows Cancel only when `JobView.cancellable` is true. Reverification exposed one remaining race: the chain's final raw-flag check could report Cancelled for a click arriving after the last step had completed and committed. The chain now records cancellation when its own loop prevents a later step, and its final check reports Cancelled only when the last-step worker actually observed the request. Per-item states and ML cancelled-log-set metadata expose completed partial work.
- **Qualifying acceptance test:** `jobs::tests::every_displayed_cancel_reaches_an_observing_worker_and_completed_work_is_never_reported_cancelled` is `CORRECTNESS`. SB-CORE-036 and its dated correction source the two required final states. The test executes both sides, proves monolithic jobs are non-cancellable, inventories all seven `run_job` registrations plus the one manual chain registration, follows each to an observer, and pins both visible Cancel surfaces.
- **Supporting tests:** `cancellable_flag_reaches_the_view_both_ways`, `cancel_counts_as_cancelled_only_once_a_worker_observes_it`, `note_cancel_observed_marks_it_for_raw_flag_readers`, and `module_run_skips_all_wells_when_cancelled` remain narrower supporting coverage.
...
- **Verdict:** `PRESENT-OK`; `PILOT-BLOCKER` (satisfied engineering contract); `DEGRADED-RESULT`; test class `CORRECTNESS`; commit state `INTEGRATED` after the requirement commit.
- **Blocker or decision:** none for Gate 2 engineering. Visual, manual and field interruption evidence remains unexercised and is not inferred from the automated inventory.
- **Next action:** retain the owned registration inventory and field-exercise a mid-write cancellation plus a too-late cancellation, confirming the UI, terminal state and persisted partial-result qualification agree.
...
- **Current source:** `docs/IP_PROVENANCE.md` defines tiers, a same-increment maintenance rule, blocking treatment and known asset/fallback routes; `CONTRACT.md` defines independent derivation and primary-source requirements. Reverification confirms the digitized chart payloads and vendor-merged endpoint library still ship under unresolved routes; the four branded theme identities CLOSED 2026-08-19 via the DEC-074 neutral route (renamed to colour themes, palettes byte-identical, old ids surviving only as migration keys - pinned by installation.rs::the_colour_themes_carry_no_client_brand_and_the_old_ids_survive_only_as_migration_keys, five mutations killed, browser-verified). The dependency slice is improved: `THIRD-PARTY-LICENSES.md` is regenerated from Cargo normal edges and npm's installed production graph, excluding hoisted development tools; it currently records 292 Rust crates and 111 npm packages, six MPL-family attention items and no undeclared licence. `tools/check.ps1` now rejects a stale generated notice. This factual inventory does not resolve the other asset classes or supply counsel's compatibility judgment, and no source scanner can prove exhaustive provenance for an origin record that was never written.
- **Qualifying acceptance tests:** none; test class is `MISSING`.
...
- **Manual evidence:** `verification-stewardship` 0/24 - unexercised.
- **Git evidence:** reachable `43fddd3` contains the patch-equivalent integration of original characterization candidate `3abf150`; the Gate 2 requirement commit refreshes and machine-checks the distributed-dependency slice without altering any protected payload or claiming legal clearance.
- **Verdict:** `PARTIAL`; `PILOT-BLOCKER`; `DATA-INTEGRITY`; test class `MISSING`; commit state `INTEGRATED`; Gate 2 outcome `BLOCKED`.
- **Blocker or decision:** the chart payloads need a counsel-approved existing route or a primary-sourced independent replacement; the endpoint library needs exact per-value construction custody or primary re-sourcing under `SB-CORE-005`; branded themes took the approved neutral route (DEC-074, done); dependency attention items need counsel review. None is inferred from a green gate.
- **Next action:** obtain those named dispositions, remove or independently re-source every uncleared shipped route, and only then add an owned release-inventory correctness test that fails on any unregistered or uncleared Tier-C asset.
