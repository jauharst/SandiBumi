...
- **Blocker or decision:** the chart payloads need a counsel-approved existing route or a primary-sourced independent replacement; the endpoint library needs exact per-value construction custody or primary re-sourcing under `SB-CORE-005`; branded themes need permission or an approved neutral/user-owned route; dependency attention items need counsel review. None is inferred from a green gate.
