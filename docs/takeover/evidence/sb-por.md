...
- **Current implementation:** still absent. `phi_dn` offers only the `AVERAGE` and `GAS_RMS` shortcuts, correctly typed by SB-POR-001 as a comparison producer and labelled quick-look by SB-POR-023.
- **Qualifying acceptance tests:** none; the evaluator is unwritten. Test class `MISSING`.
- **Implementation-ready witness**, derived by hand from B-5/B-11/B-12 and deliberately **not** read back from any code: at `rho_b` 2.30, `rho_mf` 1.00 and `phi_N` 0.25 limestone units, `phi_D` = 0.2397661; `phi_N > phi_D` so `rho_2` = 4.0; `phi_Na` = 0.6610955; `phi_Da` = -0.7543860; crossplot porosity **`phi_x` = 0.245219**. The `phi_N < phi_D` branch (B-9/B-10) must be pinned separately.
- **Manual evidence:** porosity 0/33.
- **Verdict:** `ABSENT`; `PILOT-BLOCKER`; `SILENT-WRONGNESS`; test class `MISSING`; commit state `UNIMPLEMENTED`.
- **Blocker or decision:** source is closed; **implementation pending**. One design constraint carries forward: the evaluator must be a typed deterministic method **distinct from the D-N comparison producer** - not another `OPT_XPLOT` mode - or SB-POR-023's quick-look boundary collapses the moment the real method lands beside the shortcut.
- **Next action:** implement B-5/B-6/B-7 and the B-9..B-12 pseudo-mineral branches as their own registered POR method, pin both branches with the witness above, then compare independently against the chart and shortcut discriminators.
