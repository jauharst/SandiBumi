...
- **Current implementation:** three of the four paths are already correct or vacuous. `phi_den` and `phi_dn` both call the shared `modules.rs::phit_sh_at`, which is `(RHO_DSH - RHO_SH)/(RHO_DSH - RHO_W)` using `RHO_W`, the formation-water density the requirement names. `phi_son` has no shale-porosity term at all - its `DT_SH` is only the Wyllie compaction divisor - so there is no parallel formulation to unify there. The parallel formulations are both inside protected `src-tauri/src/ssc.rs`: line 259 and line 464 each define a local `phit_sh` as `(RHOB_DSH - RHOB_SH)/(RHOB_DSH - RHOB_FL)`, using **fluid** density where this requirement requires **formation water** density. `ssc.rs` declares no `RHO_W` parameter at all (its density arguments are `RHOB_DCL`, `RHOB_DSH`, `RHOB_FL`, `RHOB_MA`, `RHOB_MAT`, `RHOB_SH`, `RHOB_WCL`), so correcting it needs both a new parameter and a formula change in a file this program may not edit. The export target, the CLY chapter's `clsr_porosity_corrected` (SB-CLY-044), does not exist in the Rust source and SB-CLY-044 is outside the approved 242-row manifest.
- **Qualifying acceptance tests:** none, and deliberately so. Test class `MISSING`. A test covering only the `modules.rs` paths would assert that the quantity is "defined once" while two parallel definitions of that exact name survive in `ssc.rs` - which is precisely the F16 conflation SB-POR-008 exists to prevent - so writing it would have converted a boundary into a false green.
- **Supporting tests:** density/D-N shale tests and `sspw_phie_removes_only_clay_bound_water` each passed, but they are not independent oracles for one another; each pins its own implementation's arithmetic, which is why neither detects the divergence.
- **Manual evidence:** porosity 0/33; shale-volume 0/17; workflow 0/23.
- **Source/parameter boundary:** `docs/PRD_v2/11_porosity.md` SB-POR-008 and F16 supply the required form and the shale-subtraction distinction. Section 5.1 supplies the divergence magnitude: fluid density ships fresh `1.00` and salt `1.10`, while formation-water density ships `1.00`. The two forms therefore agree exactly at the shipped defaults and separate only once an interpreter selects salt water - which is why this survived review. No value was invented and no parameter was substituted.
- **History/reachability:** the shared `modules.rs` helper and the two separate `ssc.rs` expressions are all integrated and reachable; the CLY consumer is absent.
- **Verdict:** `PARTIAL`; `PILOT-BLOCKER`; `SILENT-WRONGNESS`; test class `MISSING`; commit state `INTEGRATED`.
- **Blocker or decision:** `BLOCKED-BOUNDARY`. The only parallel `PHIT_SH` formulations live in prohibited `src-tauri/src/ssc.rs` at lines 259 and 464, which also exposes no formation-water parameter; and the CLY export arm's target SB-CLY-044 is deferred outside the manifest. Engineering will neither edit the protected file nor call the contract satisfied while the parallel definitions stand.
- **Next action:** Jauhar authorizes a narrow `ssc.rs` edit that adds a formation-water parameter and routes both SSC and SSPW through the shared helper while preserving their existing limited arithmetic, or explicitly re-adjudicates SB-POR-008 to the `modules.rs` paths while SSC/SSPW remain first-pilot exclusions. The CLY export arm additionally needs SB-CLY-044 admitted to the manifest or the arm re-scoped.
