...
- **Current implementation (2026-08-18):** DEC-038's constraints are DONE. `PorosityModuleRole::TypedWorkflow` types `ssc`/`sspw` (registry change only; method identities unchanged; `ssc.rs` untouched, DEC-040's narrow authorization still unused). `modules::porosity_pair_declaration_failures` enforces the ruling's both-sides gate on every catalog build: a DeterministicMethod or ComparisonProducer that does not declare all four unlimited/limited (or comparison) halves is refused BY NAME; a TypedWorkflow or LimitProducer is exempt by ROLE, not by name. `phi_son` is the single named pending exemption (`POROSITY_PAIR_PENDING`), tied to open DEC-063; the qualifying test pins the list's exact content so a second name cannot ride in silently. Density, D-N and Bateman-Konen already carry their twins; sonic still clamps its only pair in place.
- **Qualifying test (ruled half):** `modules::tests::ssc_and_sspw_are_typed_workflows_and_a_method_cannot_use_that_role_to_skip_its_twin_pair` - workflow typing with unchanged method identities; SSC/SSPW registered porosity outputs pinned to exact pre-re-typing f32 values on clean-sand and clay-point fixtures (GR-equivalent family pinned MISSING at degenerate VWSH); a METHOD with generic outputs refused naming module and each missing half; the SAME outputs under the workflow role passing; the pending list pinned to exactly `phi_son` with stale names refused. Five mutations killed on distinct assertions (re-typing reverted, method check disabled, workflow exemption removed, pending hygiene disabled, workflow arithmetic perturbed under the re-typing). The row's own whole-family test class stays `MISSING` until DEC-063 rules the sonic half.
...
- **Verdict:** `PARTIAL`; `PILOT-BLOCKER`; `DATA-INTEGRITY`; test class `MISSING`; commit state `INTEGRATED`; Gate 2 `BLOCKED-DECISION` narrowed to DEC-063: does SB-POR-009's every-sample `PHIT >= PHIE` govern the UNLIMITED sonic pair, or the limited pair only? Option (a) lets engineering finish the row with no further input; option (b) parks the sonic twin on the SB-POR-013/014/015 rebuild.
