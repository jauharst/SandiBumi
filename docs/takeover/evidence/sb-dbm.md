...
- **Current source:** `ingest.rs` requires a declared continuous style for every new LAS set. A regular declaration additionally requires a unit-typed tolerance with no default, verifies the delivery's declared STEP against the sanitized native reference samples and atomically writes declared/effective style, verdict, input tolerance, warning, gap depth and missing-row count to `import_sets`. `ancestry.rs` refuses an explicit imported-set frame read without that verdict and uses verified native reference samples rather than a synthesized row index.
