...
| 14 | Stop pushing the archive's rows through a SECOND appender - Phase 1 deleted exactly these (well, curve) pairs and Phase 2 refilled them, so the rows the archive needs are already in `computed_curves` with their own set_id. Copy them table-to-table inside the same transaction instead | real 100-well chain, two runs each side, paired in one session: archive phase **5,724ms -> 640ms (8.9x)**, write **13,630 -> 9,387ms (1.45x)**, chain **23.29 -> 19.37s (1.20x)**, before and after ranges DISJOINT | **KEPT** | Writing the archive was never expensive because of the data - it was expensive because 2M values re-crossed the Rust-to-engine boundary one row at a time. Same rows, same table, same append-only guarantee, an eleventh of the cost. ~890ms of the win is paid back by the current-store phase (the copy forces rows to materialize that were buffered), and the parts sum to the whole. `PERF-ARCHIVE-COPY-2026-08-24.md`. |
| 15 | Time the five parts of the pay summary's per-well FLAG write, rather than trusting the harness's own `stats_only` minus writing subtraction - which crosses two calls with different cache states and names the block, not the part | real 100-well delivery: flag rows **54.2%**, log-set **22.1%**, provenance **12.9%**, ancestry lookups **7.9%**, reads+summation **3.1%**, lock **0.0%** | **KEPT** | The lock is zero, so this was never contention. The petrophysics is 3%. And the 54% is misleading alone: 30.3 ms per well writes 4,686 rows at ~155k rows/s against ~2,000k for the same table, so ~2.3 ms was rows and the rest was the COMMIT. Divide a share by its row count before choosing a fix. |
| 16 | Collect every well's FLAG curves and write them in ONE transaction after the loop, the shape a chain step already uses - one commit instead of a hundred, and one archive copy instead of a hundred scans | real 100-well delivery, two runs each side: **5,605 -> 3,730 ms (1.50x)**; flag rows 3,031 -> 1,639, log-set 1,236 -> 656; 468,600 FLAG rows across 100 wells, asserted | **KEPT** | The log-set step got 1.9x faster untouched and that is reported as UNEXPLAINED, not claimed. Two behaviour notes stated rather than buried: `CompleteWellWrite`'s degradation fields became `Option` so PAYFLAG versions stay unclassified exactly as before, and the write is now all-or-nothing on failure. `PERF-PAYSUMMARY-2026-08-24.md`. |
| 17 | Move the pay summary's provenance build and already-current check off the per-well loop and through the reader pool - the last per-well round trips in that function, in a function that already prefetches its INPUTS that way | real 100-well delivery, two runs each side: **3,730 -> 2,557 ms (1.46x)**; those two phases **1,243 ms serial -> ~305 ms wall-clock (~4x)**; 468,600 FLAG rows across 100 wells, unchanged | **KEPT, with the better fix named** | It spreads the work rather than reducing it: provenance went 741 ms serial -> **6,260 ms summed across threads**, so ~8x the total work bought 4x the wall-clock, the threads contending on DuckDB's buffer manager. These are **N+1 queries** - ~700 of them for 100 wells - and parallelising N+1 makes them finish sooner without making them FEWER. Collapsing them to TWO queries would cut the work instead, very likely beating 2.56 s on one core, and is recorded as outstanding so this cannot read as the area being finished. `PERF-PAYSUMMARY-2026-08-24.md` part 2. |
| 18 | Collapse the N+1 that row 17 only spread - ask TWO questions instead of ~700, via `curve_ancestry_batch` (a faithful replacement) and `resolve_ancestry_inputs_batch` (a fast path over ONE of four resolution paths, with the per-call function as the fallback for the rest) | those two phases **~305 ms wall / ~9,800 ms thread-time -> 77 ms on one thread** (~127x less work); total elapsed **2,557 -> 2,871 ms**, i.e. 1.12x SLOWER than row 17, but **3,730 -> 2,871 (1.30x) against the pre-row-17 baseline, on 1 core instead of 8** | **KEPT, superseding row 17** | **My part-2 prediction that it would beat 2.56 s on one core was WRONG** - it lands just short. The ground was lost in the FLAG-row write, 1,484 -> 1,909 ms on byte-identical code: row 17's eight threads had been accidentally warming the buffer cache for the archive copy's unindexed full scan that follows. Hypothesis, not measurement. `curve_ancestry` lost its last production caller and is now `#[cfg(test)]` as the SPECIFICATION the batch is pinned against - deleting it would pin the batch against nothing. `PERF-PAYSUMMARY-2026-08-24.md` part 3. |
