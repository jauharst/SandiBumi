...
- D:\01. Work\2026\43. SCS - PHM\01. Data\05_Core and Petrophysical Data\OTHER REFERENCE\20221101 Manpatu Core Data\Core_data_SCS.CSV
  READ header. the reference suite-export CCAL/RCA core table format: row1 mnemonics DEPTH,K_KL_CORE_NCS_1,PHIE_CORE_NCS_1,PHIT_CORE_NCS_1,RHOG_CORE_1,SAMPLE_NUMBER_1,WELL_NAME_1; row2 units METRES,%,%,%,G/C3; then data. Companion Core_data_SDS.CSV same format. This is the phi-k input table shape for rock typing.
...
Inputs: Core CCAL at NCS (per-well NCS psi recorded, e.g. Sisi-Nubi 2600-5000 psi list on slide 20), PhiT_core, K_KL (and Kg@Swi from SCAL rel-perm), zone attribute (burial depth mSS, delta axis, facies SF1-SF8); the reference suite-CSV core table format (Core_data_SCS.CSV).
