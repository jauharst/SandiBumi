...
- D:\01. Work\00. Guidebook\02. Guidebook Geolog\FACIMAGE\2006 - the reference suite 6.6.1 - FACIMAGE; Practical Illustrated Guide.pdf
  Rabiller's FACIMAGE guide. NOT borehole image processing - it is electrofacies: MRGC clustering, KNN core-data prediction, STM log similarity, histogram upscaling. Relevant only as analog for a future auto-texture/electrofacies module (Geomage Auto Texture MRGC uses same engine). A second copy (the reference suite 6.6) sits in the same folder.
- D:\01. Work\00. Guidebook\02. Guidebook Geolog\coreanalysis.pdf
  the reference suite 20 'Core Analysis' user guide (219 pp): SCAL import, capillary-pressure corrections (closure, clay-bound water, stress), saturation-height modeling (lambda/EQR, Leverett-J, FWL solving). NOT core photos - irrelevant to Topic B, relevant to a future SCAL module. Duplicate larger copy: 'Core Analysis User Guide.pdf' in same folder.
...
Equations:
EMEX normalization (standard FMI form): button conductivity c_ij proportional to I_ij / (V_emex * G_electronic); the reference suite exposes it as EMEX_FLAG gain correction toggle. Interlacing: out[2k]=row1[k], out[2k+1]=row2[k].
...
Calibration: Excluding a pad with bad response is a display/processing option (deviated hole with floating pad). Magnetic declination must already be applied to P1AZ/HAZI (the reference suite exposes MDEC_COR toggle).
...
Inputs: Oriented array or individual pad arrays; window size (Techlog default 1 m dynamic; the reference suite default 0.3 m / 0.984252 ft); process type {Global | By array | By button}; output min/max (0-255 editable, reversible for high=resistive vs high=conductive conventions); equalization type {static|dynamic|both}; low/high percentile cutoffs (the reference suite 0/100 defaults).
...
### A15. Image porosity + binarization + sand count (the reference suite SOP methods)
