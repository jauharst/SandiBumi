...
- [x] **T-ADV-17** — SandiMin re-run, lowercase prefix, no shadow rows · `a_re_run_under_a_lowercase_prefix_leaves_no_shadow_rows` (`sandimin.rs`). Both halves of the fix hold: the prefix is canonicalized and the case-insensitive DELETE reclaims prior-casing rows. Clean.
...
| T-SHIP-04 | R-A: the project carries a format stamp | `fresh_project_is_stamped_with_current_format` asserts `format_version = FORMAT_VERSION` and a `written_by` value starting `SandiBumi ` — the exact two rows the manual `SELECT * FROM project_meta` expects. `legacy_project_without_stamp_is_stamped_on_open` covers invisible legacy stamping, and `a_newer_format_is_refused_with_both_versions_and_its_writer_while_the_project_bytes_remain_identical` proves the refusal names the complete identity and mutates no byte. |
| T-SHIP-05 | R-B: destructive migration backs up first; normal opens write nothing | `fresh_project_open_writes_no_backup` asserts step 1's absence condition. `a_destructive_upgrade_backs_up_before_writing_never_overwrites_reports_the_path_and_aborts_on_backup_failure_while_an_additive_open_takes_no_backup` drives the complete step-2 safety contract; `consecutive_destructive_upgrades_name_each_backup_for_the_source_format_it_restores` proves the recovery shelf is source-labelled (`pre-0`, then `pre-1`) rather than target-labelled. |
...
| T-ADV-17 | SandiMin re-run with a lowercase prefix: no shadow rows | assert run_sandimin with output_prefix "mm" writes the uppercase MM_* names and that a re-run replaces them case-insensitively, leaving one row per curve at a bumped version (sandimin.rs + db.rs); nothing pins the prefix canonicalization today |
...
| T-ADV-15 | SandiMin wet→dry clay converter (KKT ONWJ workflow) | no blocker — what is untested is the Apply wiring (Illite's endpoints rewritten, CEC cell set, BoundWater auto-ticked, both previews refreshing on a temperature edit); the numbers behind it are pinned by `dry_clay_matches_the_kkt_example` (src-tauri/src/sandimin.rs:3019) and `dry_clay_cec_reproduces_the_bndwat_tie` (3040) |
| T-ADV-18 | SandiMin negatives: too few components / under-determined / empty scope | no blocker — the empty-scope message is frontend; `rejects_underdetermined_request` (src-tauri/src/sandimin.rs:2658) pins step 2's refusal, but the "select at least two components" guard (sandimin.rs:1001) has no test and is a cheap B on its own |
