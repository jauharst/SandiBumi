...
- [ ] **PR #2 merge**: `docs/prd-and-security-hardening` now carries every master commit
      ...
      this PR.
- [ ] **RENAME THE FOLDER — one command, needs Claude Code closed.** Everything else is
      done (all 247 in-repo path references now say `D:\XX. SandiBumi`). Close Claude
      Code / Cursor / SandiBumi, then run:
      `powershell -ExecutionPolicy Bypass -File "D:\XX. Arshilla\tools\rename-to-sandibumi.ps1"`
      It renames the folder, repairs both git worktrees, repoints the app's recent-projects
      list and carries over the Claude memory folder. **Claude cannot do this step**: a
      process's working directory is an open handle on it, and the project folder is Claude
      Code's own working directory, so Windows refuses the rename from inside a session.
      Reopen Claude Code on `D:\XX. SandiBumi` afterwards.
