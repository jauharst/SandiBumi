...
| | |
|---|---|
| **Assets** | `src/ui/chartOverlays.ts` (19 chart definitions at digitization; **six derived definitions since DEC-082**) · `src-tauri/src/neutron_charts.rs` (Por-4 / Por-5 equivalence tables) |
...
| **Execution (2026-08-19, DEC-079/DEC-080)** | Five definitions now carry independently derived data replacing the digitized coordinates at the resolver: `por22_ta` (Wyllie), `por22_fo` (the published RHG-1980 algorithm at the paper's own constants — deliberately not a replica of the printed tracing, per DEC-079), `lith3`/`lith4` (Gardner & Dumanoir 1980 physics, DEC-079) and `lith6_mid` (ternary geometry). DEC-080 reclassifies `por20_ta`/`por20_fo` INTO the tool-response class (their neutron coordinates are the CNL response — the same data class as `por11` — so a half-derived chart stays encumbered): the retained register holds **eleven** ids and the derivable class eight. Increment 6 (same day): `lith2_thk` derived — the six Th/K boundary lines are geometry from the chart page's own printed values (p. 194), the classification published as Quirein et al. SPE 11143 (1982) Fig. 2 after Hassan-Hossin-Combaz SPWLA 1976 (both papers obtained and read); the dashed clay/feldspar lines and region label points stay digitized behind the pinned keep-list. Increment 7 (same day, DEC-081): `lith1_pek`/`lith1_pethk` reclassified INTO the retained register — the entire cited chain was obtained and read (Edmundson & Raymer 1979: Pe but **no thorium**; Quirein 1982: formulas only; Hassan 1976 and Hassan & Hossin 1975 — the root source, read from the Gallica scan of C.R. t.280 pp. 533–535, ark bpt6k5814218t: figure graphics only; Serra 1981: internal Schlumberger), so no printed numeric source exists in principle. **The register holds THIRTEEN ids; the derivable class closed COMPLETE at six of six executed — the route-2 derivation program is finished.** DEC-082 (same day): **Jauhar delivered the Gate 5 word — DELETE — and it is executed**: the thirteen vendor definitions were removed from `chartOverlays.ts`; `chartOverlayPolicy.ts` now carries the deletion record (`DELETED_VENDOR_OVERLAY_IDS`) and `tools/chart-derivation.test.mjs` pins fail-closed that no deleted id reappears and all six derived definitions survive. The deleted bytes exist only in git history — the record this register requires. What ships from the chartbook work is now: the six derived definitions (published equations + printed page values, each cited in `tools/gen-derived-overlays.mjs`) and their keep-listed digitized remnants (apparent-position mineral points and annotation lines), which remain under CLAIM-013 for counsel. |
| **Pre-ship posture (2026-08-20, DEC-083)** | Counsel could not be obtained in the pre-ship timeframe; Gate 2 is formally closed with CLAIM-013 **DEFERRED BY NAME to first commercial sale**. The deferred set: the six derived definitions, their keep-listed digitized remnants, and the Por-4/Por-5 equivalence tables (`neutron_charts.rs`) — never inside the thirteen-id ruling, still shipping because the neutron-porosity module math depends on them. The deferral is pinned by `tools/release-inventory.test.mjs`, which also sweeps the tree for the chartdig banner so no digitized payload can ship unregistered. DEC-083 is a posture, not clearance. |

### 2.2 Vendor-derived parameter defaults (PRD R2)

| | |
|---|---|
| **Assets** | `src-tauri/src/sandimin.rs` (renamed from `multimin2.rs`, 2026-08-20) — the 27-entry component `LIB` and its endpoint matrix across 14 tool keys (RHOB, NPHI, DT, GR, PEF, U, THOR, POTA, URAN, VP, VS, EPT, EATT, SIGMA) |
...
| **In-code note** | `multimin2.rs:2048` (historical; the file is `sandimin.rs` since 2026-08-20 and the string was replaced under DEC-078) — "Merged reference/IP default library, in IP's mineral-dropdown order". |
...
| **Outcome (2026-08-19, DEC-078)** | Done, in the owner-adjudicated form: Jauhar ruled the library **his default library**, and per-value custody now ships in code (`sandimin.rs` `LibRow::src` + `Component::endpoint_sources`, carried through the SandiMin dialog and every run record's `params_json`). Every value was verified against his copy of the *Schlumberger Log Interpretation Charts* (2013): values the book states in print (Appendix B pp. 279–280, Appendix C p. 281, chart Por-1 p. 212) cite the page; every other value — including all near-misses, where the printed number differs from the library's — is owner-attributed to DEC-078. Shipped strings no longer claim vendor-install custody; **this section remains the custody history and is deliberately not rewritten**. CLAIM-012's counsel review stays open as a first-sale item. |
...
| | |
|---|---|
| **Assets** | `ssc.rs` (SSC / SSPW) · `lrlc.rs` (RtC / IMTS) · `modules.rs` saturation, porosity, VSH and permeability families · `satheight.rs` · `sandimin.rs` solver (renamed from `multimin2.rs`) |
...
**Pre-ship status (2026-08-20, DEC-083):** every "First sale" row above is formally DEFERRED to
first commercial sale — counsel could not be obtained in the pre-ship timeframe. The deferral is
pinned by `tools/release-inventory.test.mjs`; the question for the lawyer is unchanged.
