...
- **end-to-end** — a WebDriver spec drives the BUILT app through it. Optional, never part of the
  green gate (`npm run test:e2e`, see `docs/e2e_harness.md`); it needs a built binary and a
  WebDriver stack, and a gate that can fail for reasons unrelated to the code is one people learn
  to ignore. These lines always end by naming what is **not** covered.
...
**Every heading now carries a one-word tag** summarising the above — `[YOURS]`, `[PART-AUTOMATED]`
or `[GATE-PINNED]` — so you can scan the file instead of reading each coverage line to find out
whether it is worth your evening. **Start with the `[YOURS]` list under "Start here" below.**
...
## Start here — what is still yours (regenerated by `tools/testplan-tag-coverage.py`)
...
| --- | ---: | ---: | ---: | ---: | ---: |
| SHELL — Shell & project lifecycle | **0** | 0 | 0 | 18 | 18 |
| IMP — Data import & export | **6** | 1 | 8 | 2 | 17 |
| WELL — Wells, groups, tops & zones | **8** | 7 | 2 | 0 | 17 |
| PREP — Prep & conditioning modules | **7** | 1 | 11 | 0 | 19 |
| PETRO — Core petrophysics modules | **11** | 2 | 6 | 0 | 19 |
| ADV — Advance tab | **12** | 2 | 5 | 0 | 19 |
| RT — Rock typing, HFU, Pc, SHF, facies | **12** | 2 | 4 | 0 | 18 |
| BATCH — Batch & field-scale tools | **10** | 6 | 3 | 0 | 19 |
| MLEQ — ML, equations & curve management | **12** | 6 | 0 | 0 | 18 |
| PLOT — Plots, viewers & curve editing | **19** | 0 | 1 | 0 | 20 |
| REP — Reporting & database access | **8** | 5 | 6 | 0 | 19 |
...
- **WELL** (8) — T-WELL-07-14
- **PREP** (7) — T-PREP-04, 06-08, 12, 17, 19
- **PETRO** (11) — T-PETRO-01, 04, 06-07, 10-11, 15-19
- **ADV** (12) — T-ADV-02-06, 08-09, 12, 14-16, 19
- **RT** (12) — T-RT-02-04, 06, 09-15, 17
- **BATCH** (10) — T-BATCH-02-03, 05, 09, 11-13, 15, 18-19
- **MLEQ** (12) — T-MLEQ-03-04, 06-08, 10-13, 15, 17-18
- **PLOT** (19) — T-PLOT-01-18, 20
- **REP** (8) — T-REP-04-05, 08, 10-11, 13, 15, 19
...
### What you have already marked

27 of the 250 carry a mark: **20 Pass**, **5 Fail**, **2 Blocked**. All 20 passes are SHELL,
so that section is done. Every Fail and Blocked is in IMP, and **all seven predate the import
work of 2026-07-30/31** — each one now carries an Update or a STALE block saying what changed:

| | Your note, shortened | What happened since |
| --- | --- | --- |
| T-IMP-03, -04 | "where do u provide dup_depth.las?" | The exemplars did not exist. They do now: `bad_dup_depth.las`, `bad_null_depth.las`, `bad_truncated.las` in `dataset for test/examples/`. Both Blocked marks are answerable. |
| T-IMP-05 | Core/SCAL "does nothing" with no well selected | `needWell.ts` turned that silent refusal into a named dialog. **The test's own Expected line is stale**, and your Fail is the only record saying this is broken. |
| T-IMP-06 | DLIS imported but the well did not show | Partly built — read the Update on the test. |
| T-IMP-07, -10 | Core import should read the well name from the file | Core import v2 does: probe → confirm the mapping → commit, routing rows by a WELL column. |
| T-IMP-08 | "how is duplicated data managed" | Your note became the spec. Delivery sets: an import never overwrites, it lands as a new named set. |

**Re-running those seven is probably the cheapest hour in the plan** — several are likely a
Pass now, and until they are re-marked the tally reports a health the app no longer has.
...
### T-SHELL-01 — App launch (dev run)  [PART-AUTOMATED]
...
### T-SHELL-02 — Ribbon tab walk + overflow chevrons  [PART-AUTOMATED]
...
### T-SHELL-03 — UI language switch EN → ID → SU → JV → EN  [PART-AUTOMATED]
...
### T-SHELL-04 — Theme switching, all 8 themes, live repaint  [YOURS]
...
### T-SHELL-05 — New Project + data isolation  [YOURS]
...
### T-SHELL-06 — Open Project + Recent list (incl. missing file)  [YOURS]
...
### T-SHELL-07 — Save Project As = backup copy  [GATE-PINNED]
...
### T-SHELL-08 — Clean relaunch restores last project + workspace  [YOURS]
...
### T-SHELL-09 — NEGATIVE: project switch refused while a chain runs  [GATE-PINNED]
...
### T-SHELL-10 — Sessions: Save Session As / Open Session / delete  [PART-AUTOMATED]
...
### T-SHELL-11 — Quiet Ctrl+S re-save + Escape closes ribbon menus  [PART-AUTOMATED]
...
### T-SHELL-12 — Dirty-state ● indicators  [PART-AUTOMATED]
...
### T-SHELL-13 — Undo/Redo with live labels (+ History cross-check)  [PART-AUTOMATED]
...
### T-SHELL-14 — Processing History panel: entries, export, clear  [YOURS]
...
### T-SHELL-15 — History attribution: single-well vs batch module run  [PART-AUTOMATED]
...
### T-SHELL-16 — Global well pin 📌 (workspace-follow vs working-pane)  [YOURS]
...
### T-SHELL-17 — Interaction guards: right-click, reload, armed number fields  [YOURS]
...
### T-SHELL-18 — Crash resilience: autosave + recovery dialog (run LAST)  [YOURS]
...
### T-IMP-01 — LAS batch import, multiple files at once  [GATE-PINNED]
...
### T-IMP-02 — Re-import a same-named LAS → duplicate-name warning, separate record  [PART-AUTOMATED]
...
### T-IMP-03 — Malformed LAS: duplicated depth section imports with a dropped-rows warning  [GATE-PINNED]
...
### T-IMP-04 — Malformed LAS: all-null depth column and truncated last row → clean error, no orphan well  [GATE-PINNED]
...
### T-IMP-05 — No-well-selected guards and cancel mid-dialog leave no side-effects  [YOURS]
...
### T-IMP-10 — Tops CSV: multi-well WELL column, single-well file, unmatched + blank WELL cells  [GATE-PINNED]
...
### T-IMP-12 — Deviation survey import: TVD/TVDSS computed; duplicate-MD survives; TVD not yet consumable  [GATE-PINNED]
...
### T-IMP-13 — Well locations import → wells post on the Field Map at UTM coordinates  [YOURS]
...
### T-IMP-14 — SCAL Pc/Sw import: multi-file, auto-detect, Leverett-J fit reported  [PART-AUTOMATED]
...
### T-IMP-15 — LAS export: NaN→−999.25, computed curves included, mixed-case name exports real values  [GATE-PINNED]
...
### T-IMP-16 — Round-trip: export → fresh project → re-import → values identical  [GATE-PINNED]
...
### T-IMP-17 — Cross-checks: open panes refresh on import (dataVersion) and repaint on theme switch  [YOURS]
...
### T-WELL-01 — Object tree: click activates, 📌 pin mode drives the workspace  [PART-AUTOMATED]
...
### T-WELL-02 — Multi-select: Ctrl-click, Shift-click range, ⇄ invert, plain-click clear  [PART-AUTOMATED]
...
### T-WELL-03 — Multi-selection feeds a batch dialog's "Selection" scope, live  [PART-AUTOMATED]
...
### T-WELL-04 — Well Groups manager: create, edit membership, rename, delete  [PART-AUTOMATED]
...
### T-WELL-05 — Active group scopes the tree and freshly opened batch dialogs  [PART-AUTOMATED]
...
### T-WELL-06 — NEGATIVE: already-open batch dialog does NOT re-scope on group switch  [PART-AUTOMATED]
...
### T-WELL-07 — Field Map smoke: markers, pan/zoom, Fit, info line, group ring, theme, auto-fit  [YOURS]
...
### T-WELL-08 — Draw polygon → assign enclosed wells to a NEW group  [YOURS]
...
### T-WELL-09 — Polygon editing, union into EXISTING group, Esc/Clear negatives  [YOURS]
...
### T-WELL-10 — Tops pane: click a top windows plots to its interval  [YOURS]
...
### T-WELL-11 — Tops editor: 🏷 add a top, undo, History, cross-pane sync  [YOURS]
...
### T-WELL-12 — Tops editor: drag-move, edit/rename/recolor/delete, stratigraphic crossing warning  [YOURS]
...
### T-WELL-13 — Top autocorrelation: propagate by GR shape, untick, apply, batch undo  [YOURS]
...
### T-WELL-14 — Autocorrelation negatives: no tops, bad curve, no targets  [YOURS]
...
### T-WELL-15 — Zones pane: From Tops, add/update/delete, invalid input, History  [PART-AUTOMATED]
...
### T-WELL-16 — Per-zone parameter override actually drives a module run  [GATE-PINNED]
...
### T-WELL-17 — NEGATIVE: degenerate zone override (TD_BHT = 0) reports honestly, no garbage  [GATE-PINNED]
...
### T-PREP-01 — Module dialog machinery smoke: dropdown, pane form, "(none)" optional input  [PART-AUTOMATED]
...
### T-PREP-02 — Formation Temperature: GRADIENT and BHT modes  [GATE-PINNED]
...
### T-PREP-03 — Formation Temperature negative: TD_BHT ≤ 0 zone override yields MISSING, not ±Infinity  [GATE-PINNED]
...
### T-PREP-04 — Pre-Calculation: FTEMP/FPRESS/RMF/CT/CXO plausible on a known well  [YOURS]
...
### T-PREP-05 — Pre-Calculation: degC metric mode + per-zone gradient kink  [GATE-PINNED]
...
### T-PREP-06 — Bad-Hole QC Flag: washout flagging vs DRHO/CALI  [YOURS]
...
### T-PREP-07 — Data Conditioning Flags: coal / tight / crossover / shoulder on the log view  [YOURS]
...
### T-PREP-08 — Neutron Matrix Conversion: LS/SS/DOL + chart spot-check  [YOURS]
...
### T-PREP-09 — GR Hole-Size Correction  [GATE-PINNED]
...
### T-PREP-10 — Density Hole-Size Correction  [GATE-PINNED]
...
### T-PREP-11 — Neutron Environmental Correction: computed-only FTEMP contract  [GATE-PINNED]
...
### T-PREP-12 — Gas Correction WITH precalc + condflag: plausible de-gassed density  [YOURS]
...
### T-PREP-13 — Gas Correction negatives: no condflag → hard error; no precalc → honest all-NaN report  [GATE-PINNED]
...
### T-PREP-14 — GR Normalization: P3/P97 alignment across two wells  [GATE-PINNED]
...
### T-PREP-15 — MASK machinery: BADHOLE mask changes gr_normalize percentiles and blanks outputs  [GATE-PINNED]
...
### T-PREP-16 — Synthetic Log (KNN Predict): fill a gap, then the masked-washout case  [GATE-PINNED]
...
### T-PREP-17 — Depth Shift: block shift + dialog range validation  [YOURS]
...
### T-PREP-18 — Splice Curves: run-to-run splice at depth  [GATE-PINNED]
...
### T-PREP-19 — Cross-checks: multi-well scope, Processing lines, History attribution, provenance, live refresh, theme  [YOURS]
...
### T-PETRO-01 — vsh_gr linear smoke run  [YOURS]
...
### T-PETRO-02 — vsh_gr nonlinear options + version N+1  [GATE-PINNED]
...
### T-PETRO-03 — vsh_gr invalid parameters (negative)  [PART-AUTOMATED]
...
### T-PETRO-04 — vsh_dn crossplot VSH + VSH_DN_FLAG  [YOURS]
...
### T-PETRO-05 — vsh_dn degenerate-triangle regression (no ±Infinity)  [GATE-PINNED]
...
### T-PETRO-06 — phi_den density porosity incl. shale branch  [YOURS]
...
### T-PETRO-07 — phi_dn crossplot porosity, AVERAGE vs GAS_RMS  [YOURS]
...
### T-PETRO-08 — phi_son Wyllie/RHG + OPT_CP compaction correction  [PART-AUTOMATED]
...
### T-PETRO-09 — phimax constant + TVDSS-trend porosity ceiling  [GATE-PINNED]
...
### T-PETRO-10 — sw_arch smoke + coal/tight zero-porosity guard  [YOURS]
...
### T-PETRO-11 — sw_indo vs sw_sim vs sw_arch on the same interval  [YOURS]
...
### T-PETRO-12 — RT = 0 null-streak regression (no +Infinity in unlimited Sw)  [GATE-PINNED]
...
### T-PETRO-13 — zone parameter override: RW in one zone only  [GATE-PINNED]
...
### T-PETRO-14 — perm_wyllie_rose, all OPT_WR variants  [GATE-PINNED]
...
### T-PETRO-15 — perm_coates default constant  [YOURS]
...
### T-PETRO-16 — perm_transform vs core φ–k + overflow regression  [YOURS]
...
### T-PETRO-17 — thin_bed_ts Thomas-Stieber decomposition  [YOURS]
...
### T-PETRO-18 — missing input curve + Cancel mid-batch (negative)  [YOURS]
...
### T-PETRO-19 — provenance, History attribution, live refresh, theme repaint  [YOURS]
...
### T-ADV-01 — Advance tab smoke: all flagship buttons render  [PART-AUTOMATED]
...
### T-ADV-02 — SSC run with LQR defaults: wiring + cross-checks  [YOURS]
...
### T-ADV-03 — SSC domain validation: closure, porosity ladder, bound-water split  [YOURS]
...
### T-ADV-04 — SSC GR-equivalent family (\*\_GR): eyeball closely  [YOURS]
...
### T-ADV-05 — SSC negative tests: bad param, empty scope, no-GR well  [YOURS]
...
### T-ADV-06 — SSC re-run: versioning + open-plot refresh (dataVersion)  [YOURS]
...
### T-ADV-07 — SSPW run: PHR-standard porosity ladder  [GATE-PINNED]
...
### T-ADV-08 — SW-RtC after SSC: corrected Rt, Sw vs Indonesia  [YOURS]
...
### T-ADV-09 — SW-IMTS after SSC: iterative Waxman-Smits-family Sw  [YOURS]
...
### T-ADV-10 — RtC/IMTS on an SSPW-only well: SSPW fallback  [GATE-PINNED]
...
### T-ADV-11 — RtC on a well with NO porosity curve at all: honest failure  [GATE-PINNED]
...
### T-ADV-12 — Saturation-Height (Leverett) with SCAL-fitted A/B  [YOURS]
...
### T-ADV-13 — Saturation-Height on a DEVIATED well: height is measured from the survey  [GATE-PINNED]
...
### T-ADV-14 — SandiMin pane smoke + fluid autofill from precalc (incl. well-switch race)  [YOURS]
...
### T-ADV-15 — SandiMin wet→dry clay converter (KKT ONWJ workflow)  [YOURS]
...
### T-ADV-16 — SandiMin full run: closure, RECON, vs deterministic answers  [YOURS]
...
### T-ADV-17 — SandiMin re-run with a lowercase prefix: no shadow rows  [GATE-PINNED]
...
### T-ADV-18 — SandiMin negative tests: too few components / under-determined / empty scope  [PART-AUTOMATED]
...
3. Restore tools; set **Apply to wells** to ★ Pinned with nothing pinned; **Run** → "No wells in scope — pick a group, pin/select wells, or choose All".
   ...
   **Automated coverage - end-to-end (pile C, 2026-08-05):** `moduledialog.e2e.mjs` "refuses an
   empty scope, and starts no run" covers **step 3 only**. That guard is nine lines in a click
   handler shared by every module pane, and the assertion that matters is not the message but that
   NOTHING RAN — a dialog that prints a complaint and then runs anyway is the failure worth
   catching, and only a before/after comparison of what is stored separates those.
   **Not covered — steps 1 and 2, which are the SandiMin-specific half:** "select at least two
   components" and the under-determined refusal. Both are yours, and step 2 is the one to spend
   time on: an under-determined solve does not fail, it returns a mineralogy that fits the logs
...
### T-ADV-19 — Theme switch repaints open Advance panes  [YOURS]
...
### T-RT-01 — Smoke: Rock Typing ribbon group lists all four modules and opens their panes  [PART-AUTOMATED]
...
### T-RT-02 — Smoke: the four rock-typing workspace panes exist in the ＋ add-panel menu  [YOURS]
...
### T-RT-03 — Rock Typing (FZI / R35 / PGS) run, GHE method: outputs, catalog provenance, History, plot refresh  [YOURS]
...
### T-RT-04 — Re-run with METHOD = winland_port: port classes and version N+1 (no overwrite)  [YOURS]
...
### T-RT-05 — Negative: run rocktyping on a well with no permeability curve  [GATE-PINNED]
...
### T-RT-06 — Lucia Rock-Fabric Number on a well with carbonate stringers  [YOURS]
...
### T-RT-07 — Rock Type from Cutoffs: RT_LOG ladder + inconsistent-cutoff behavior  [GATE-PINNED]
...
### T-RT-08 — Pittman Pore-Throat Radii: r10–r75 family + APEX selector  [GATE-PINNED]
...
### T-RT-09 — HFU Clustering (Ward) on core φ-k: table, crossplot, histogram, highlight, theme repaint  [YOURS]
...
### T-RT-10 — HFU Clustering: histogram method, K cap, and the no-core negative  [YOURS]
...
### T-RT-11 — Pc Fit (Thomeer): per-plug fit, Hg-equivalent standardization, Swanson-k suppression  [YOURS]
...
### T-RT-12 — SHF Fit — Cuddy FOIL with FWL scan  [YOURS]
...
### T-RT-13 — SHF Fit — Brooks-Corey and Skelt-Harrison forms  [YOURS]
...
### T-RT-14 — Negative: SHF Fit with a starved point cloud  [YOURS]
...
### T-RT-15 — Facies Tie-in: RT_LOG vs reference RT confusion matrix + purity  [YOURS]
...
### T-RT-16 — Legacy Multimin: deprecated entry filtered from the Workflow step picker (audit-fix regression)  [PART-AUTOMATED]
...
### T-RT-17 — Legacy Multimin v1 run (via a pre-existing saved chain): VOL\_\* + RECON_ERR outputs  [YOURS]
...
### T-RT-18 — Legacy Multimin RECON_ERR at exactly 3 tools (known QC blindness)  [GATE-PINNED]
...
### T-BATCH-01 — Workflow Builder smoke: pane opens, step picker clean  [PART-AUTOMATED]
...
### T-BATCH-02 — Compose and run the 4-step chain across 5+ wells with live progress  [YOURS]
...
### T-BATCH-03 — Chain outputs: Curve Catalog provenance, versioning, open-plot refresh  [YOURS]
...
### T-BATCH-04 — Save, reload, and delete the chain as a workflow document  [PART-AUTOMATED]
...
### T-BATCH-05 — Cancel mid-chain: quick stop, honest status, plots still refresh  [YOURS]
...
### T-BATCH-06 — Workflow negatives: no steps, empty scope, one broken well  [PART-AUTOMATED]
...
### T-BATCH-07 — Cutoffs & Pay Summary: flags, table sanity, History, PAYFLAG version  [PART-AUTOMATED]
...
### T-BATCH-08 — Pay Summary negatives: PERM cutoff without PERM, bare well, per-well isolation  [GATE-PINNED]
...
### T-BATCH-09 — Cutoff Sensitivity sweep: pick a VSH cutoff and reconcile with Pay Summary  [YOURS]
...
### T-BATCH-10 — Cutoff Sensitivity: NTG stays ≤ 1 with a mid-sample zone/DST boundary  [PART-AUTOMATED]
...
### T-BATCH-11 — Cutoff Sensitivity crossplot + DST overlay + invalid-range negative  [YOURS]
...
### T-BATCH-12 — Field Dashboard: grid, sort, box plots, CSV export, read-only  [YOURS]
...
### T-BATCH-13 — Monte Carlo smoke: default chain, 2 uncertain parameters, percentile ordering  [YOURS]
...
### T-BATCH-14 — Monte Carlo seed reproducibility  [PART-AUTOMATED]
...
### T-BATCH-15 — Monte Carlo sensitivity: Spearman ranks + tornado sweep  [YOURS]
...
### T-BATCH-16 — Monte Carlo PERM cutoff with chain-produced PERM  [GATE-PINNED]
...
### T-BATCH-17 — Cross-check: Monte Carlo vs Workflow chain with a bad-hole MASK  [GATE-PINNED]
...
### T-BATCH-18 — Monte Carlo negatives: empty scope, dry well, cancel mid-run  [YOURS]
...
### T-BATCH-19 — Monte Carlo plots on pane resize and live theme swap  [YOURS]
...
### T-MLEQ-01 — ML pane opens with the full form (smoke)  [PART-AUTOMATED]
...
### T-MLEQ-02 — Inspector opens; Python engine status is shown  [PART-AUTOMATED]
...
### T-MLEQ-03 — Python equation PHIE_TEST = PHIT × 0.9 on 2 wells  [YOURS]
...
### T-MLEQ-04 — Rhai equation (legacy per-sample engine)  [YOURS]
...
### T-MLEQ-05 — Equation negatives: unsaved run, syntax error, unresolvable input  [PART-AUTOMATED]
...
### T-MLEQ-06 — Equation re-run: open plots refresh, version v2 kept  [YOURS]
...
### T-MLEQ-07 — Electrofacies (K-means), K=4, block track + theme repaint  [YOURS]
...
### T-MLEQ-08 — Electrofacies (GMM, soft): FACIES_GMM + FPROB  [YOURS]
...
### T-MLEQ-09 — Facies negative: well with no usable input curves  [PART-AUTOMATED]
...
### T-MLEQ-10 — ML regression: predict DT from GR/RHOB/NPHI (+ leaderboard)  [YOURS]
...
### T-MLEQ-11 — ML classification: ML_CLASS + ML_CLASS_PROB  [YOURS]
...
### T-MLEQ-12 — ML clustering: K-Means k=4, silhouette, plots refresh  [YOURS]
...
### T-MLEQ-13 — ML dimensionality reduction: PCA  [YOURS]
...
### T-MLEQ-14 — ML negatives + missing bad-hole Mask  [PART-AUTOMATED]
...
### T-MLEQ-15 — ML pane list staleness while open  [YOURS]
...
### T-MLEQ-16 — Curve Catalog: provenance, statistics, search/sort  [PART-AUTOMATED]
...
### T-MLEQ-17 — Restore an old constellation version  [YOURS]
...
### T-MLEQ-18 — Delete a version: history pruned, current values untouched  [YOURS]
...
### T-PLOT-01 — Open a Log View (smoke)  [YOURS]
...
### T-PLOT-02 — Depth scale, zoom, pan (true 1:N)  [YOURS]
...
### T-PLOT-03 — Cursor readout with units + track scoping  [YOURS]
...
### T-PLOT-04 — Track resize, reorder, curve drag between tracks, scale edit  [YOURS]
...
### T-PLOT-05 — Layout Properties dialog + Save Layout  [YOURS]
...
### T-PLOT-06 — FACIES block track  [YOURS]
...
### T-PLOT-07 — Tops overlay, tops editing, interval windowing everywhere  [YOURS]
...
### T-PLOT-08 — Core-point overlays in the log view  [YOURS]
...
### T-PLOT-09 — Pin / follow-well behavior (side-by-side wells)  [YOURS]
...
### T-PLOT-10 — Histogram: bins, overlays, percentiles, picks → zone parameters, templates  [YOURS]
...
### T-PLOT-11 — Crossplot: log axes, regression (all models × methods), Z-color  [YOURS]
...
### T-PLOT-12 — Crossplot overlays: N-D chartbook, matrix points, core, Thomas-Stieber  [YOURS]
...
### T-PLOT-13 — Crossplot parameter handle, click-pick, zoom-to-cursor  [YOURS]
...
### T-PLOT-14 — Pickett plot: two-point pick ⇄ typed M/Rw, Z-color, axis config  [YOURS]
...
### T-PLOT-15 — Correlation: strips, tops connected, flatten, TVDSS, contacts, live well list  [YOURS]
...
### T-PLOT-16 — Synchronized hover crosshair across all open plots  [YOURS]
...
### T-PLOT-17 — Plot export: Copy / Image / Print + per-plot right-click menu  [YOURS]
...
### T-PLOT-18 — Curve Edit dialog: all five ops, bit-exact undo, History  [YOURS]
...
### T-PLOT-19 — Curve Edit negative tests (invalid input, stale undo)  [GATE-PINNED]
...
### T-PLOT-20 — dataVersion refresh preserving viewport; theme repaint; clean dispose  [YOURS]
...
### T-REP-01 — Composite & Report panes open, follow the selected well  [PART-AUTOMATED]
...
### T-REP-02 — Composite render: layout, print scale, page size, pagination  [GATE-PINNED]
...
### T-REP-03 — Composite depth window + invalid window (negative)  [PART-AUTOMATED]
...
### T-REP-04 — Composite export SVG  [YOURS]
...
### T-REP-05 — Composite export PDF, verify against on-screen log view  [YOURS]
...
### T-REP-06 — Report render: cover, methodology, zone params, pay summary, composite pages  [GATE-PINNED]
...
### T-REP-07 — Methodology table edit persists as report_template document  [PART-AUTOMATED]
...
### T-REP-08 — Report export single PDF  [YOURS]
...
### T-REP-09 — "Tables only" mode  [GATE-PINNED]
...
### T-REP-10 — Cross-check: report render writes FLAG\_\* curves; catalog + open plots refresh  [YOURS]
...
### T-REP-11 — Cross-check: composite/report exports in Processing History  [YOURS]
...
### T-REP-12 — Batch export, one PDF per well, with a broken well in scope  [PART-AUTOMATED]
...
### T-REP-13 — Export cancels and empty scope (negative)  [YOURS]
...
### T-REP-14 — DB Inspector: browse all 8 tables, page through  [GATE-PINNED]
...
### T-REP-15 — DB Inspector: double-click edits in wells, standard_curves, zone_params — persist, refresh, undo  [YOURS]
...
### T-REP-16 — DB Inspector negatives: bad input, stale-row edit, read-only Aux Data  [GATE-PINNED]
...
### T-REP-17 — SQL Query: starter query + provenance join computed_curves ↔ log_sets  [PART-AUTOMATED]
...
### T-REP-18 — SQL Query rejects writes (negative)  [GATE-PINNED]
...
### T-REP-19 — Cross-check: theme switch repaints reporting & DB panes  [YOURS]
...
### T-AUX-01 — Performance monitor pane opens and updates live  [PART-AUTOMATED]
...
### T-AUX-02 — Help tool: ribbon button and right-click context help  [PART-AUTOMATED]
...
### T-AUX-03 — Well Header dialog: TD/KB prefill, surface X/Y survival, edit one field  [PART-AUTOMATED]
...
### T-AUX-04 — Highlight tool: create bands, pan/zoom tracking, persistence  [YOURS]
...
### T-AUX-05 — Highlight edit / delete / Convert to zone / undo chain  [YOURS]
...
### T-AUX-06 — Well-diagram track: COMPLETION/PERFORATION import + log-view rendering  [YOURS]
...
### T-AUX-07 — Well-diagram track in Composite/Report output + old-layout compatibility  [GATE-PINNED]
...
### T-AUX-08 — Fluid-contacts editor: add/edit/delete at well, field and global scope  [YOURS]
...
### T-AUX-09 — Contacts MD ↔ TVDSS flattening across a deviated well  [YOURS]
...
### T-AUX-10 — Processing panel live detail: per-well progress, step-boundary text, Cancel  [YOURS]
...
### T-AUX-11 — Processing panel bulk same-failure summary + module-form one-liner  [YOURS]
...
### T-AUX-12 — Workflow Grid inspector: Set-all RW / Mask, amber overrides, saved-JSON stability  [YOURS]
...
### T-AUX-13 — Undo/redo depth: full walk down and up, redo invalidation  [YOURS]
...
### T-AUX-14 — Two log views of the SAME well: independent viewports, shared refresh  [YOURS]
...
### T-AUX-15 — Pinned wells (★) as a batch-run scope  [PART-AUTOMATED]
...
### T-AUX-16 — Equation negative: Python in-place output guard  [YOURS]
...
### T-AUX-17 — Equation negative: runtime error mid-batch with per-well isolation  [PART-AUTOMATED]
...
### T-AUX-18 — Importer negative: corrupt/unparsable DLIS  [YOURS]
...
### T-AUX-19 — Importer negatives: locations CSV blank-WELL rows; SCAL fluid-system guard  [YOURS]
...
### T-AUX-20 — i18n depth: Bahasa Indonesia inside dialogs + PDF header policy  [YOURS]
...
### T-INT-01 — Fresh project + multi-LAS import (canonical workflow, step 1)  [YOURS]
...
### T-INT-02 — Duplicate LAS re-import warns (negative)  [PART-AUTOMATED]
...
### T-INT-03 — Tops import → zones from tops (+ empty-well negative)  [GATE-PINNED]
...
### T-INT-04 — Conditioning prep: gr_normalize → badhole → condflag → precalc  [YOURS]
...
### T-INT-05 — Interpretation chain by hand: vsh → phi → sw → perm, each feeding the next  [YOURS]
...
### T-INT-06 — Negative trio: missing curve, out-of-range param, empty scope  [PART-AUTOMATED]
...
### T-INT-07 — Cutoffs & Pay Summary + History/flags cross-check  [YOURS]
...
### T-INT-08 — Composite + Report PDF; report numbers must equal the on-screen pay summary  [YOURS]
...
### T-INT-09 — Well-group scoping end-to-end (3-well group)  [PART-AUTOMATED]
...
4. **Run**; then per well check the Curve Catalog: the 3 members have new VSH; the 4th well must NOT.
   ...
   **Automated coverage - end-to-end (pile C, 2026-08-05):** `wellgroups.e2e.mjs` is built around
   this test and covers step 4, the one that costs data: "a run scoped to the group writes curves
   to members and not to the outsider". It also pins that exactly one group can be active and that
   saving membership REPLACES rather than appends. It runs a group of **2 of 3** rather than the
   plan's 3 of 4 — the harness uses the repo's own `SANDI-*` fixtures only, never a real project —
   which proves the same thing: a member gets the curve, a non-member does not.
   **Not covered:** step 2's status line, step 3's scope row defaulting to Group mode in a pane
   opened AFTER activation (that half is `scope.e2e.mjs` under T-WELL-05), and the History entry's
   wording. Those three are the display, which is what your click-through is for.
   **Result — T-INT-09:**
...
### T-INT-10 — Switch active group WITH a batch pane already open (negative)  [YOURS]
...
### T-INT-11 — Constellation versioning round-trip: two vsh_gr runs, restore v1, downstream consumes it  [GATE-PINNED]
...
### T-INT-12 — Session round-trip: busy workspace → save → close app → reopen → restore  [YOURS]
...
### T-PERF-01 — App launch time on the big field project  [YOURS]
...
### T-PERF-02 — Single-module run across ALL wells: UI responsiveness + Cancel  [YOURS]
...
### T-PERF-03 — Full workflow chain across all wells: speed + live progress  [YOURS]
...
### T-PERF-04 — Cancel a 540-well chain mid-run  [YOURS]
...
### T-PERF-05 — Field Dashboard compute on all wells  [YOURS]
...
### T-PERF-06 — Correlation with 20+ strips  [YOURS]
...
### T-PERF-07 — Crossplot dense-cloud pan/zoom after the memoized-color fix  [YOURS]
...
### T-PERF-08 — DB size / WAL behavior + force-kill while IDLE (crash recovery)  [YOURS]
...
### T-SHIP-01 — Packaged app launches under the hardened CSP  [YOURS]
...
### T-SHIP-02 — CSP-sensitive features in the packaged app  [YOURS]
...
### T-SHIP-03 — R30: missing perm curve fails loudly, never computes on GR  [GATE-PINNED]
...
### T-SHIP-04 — R-A: the project carries a format stamp  [GATE-PINNED]
...
### T-SHIP-05 — R-B: destructive migration backs up first; normal opens write nothing  [GATE-PINNED]
...
### T-SHIP-06 — The green gate from your own shell  [YOURS]
...
### T-SHIP-07 — R-C: a normal close must not lose recent writes (added 2026-07-29)  [YOURS]
