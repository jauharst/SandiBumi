...
all — now all three mirror the Rhai sibling (Ok / Failed / Failed).
Plus a requested feature —
**V6 Raincloud** (PtitPrince-style half-violin + box + rain in the Vega panel). Each carries a REVIEW
"Try:" line; see the R6–R27 table below and REVIEW Rounds 61–86.

**Push state (2026-07-24):** everything through this update — both halves plus the whole R1–R27 fix
...
leak), **R26** its third (the DB-Inspector reload race → wrong-well write, #6) and **R27** its fourth (the
Python-equation progress/failure blackout, #9). Note F5's own list is partly stale against the R-chain: its
...
picked. Remaining browser-independent F5 candidates: #16 inspector CodeMirror leak, #10 Tops-pane reload
race (same shape as R26), #18 correlation Wells-dropdown duplicate stacking. What still needs the live app: the invasive DB-lock
