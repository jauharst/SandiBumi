...
function renderRust(registry, digest) {
  ...
  const families = registry.families.map((family) =>
    `    FamilySpec { family: ${rustString(family.family)}, canonical_unit: ${rustString(family.canonical_unit)}, quantity_kind: QuantityKind::${kindById.get(family.quantity_kind)}, aliases: &[${family.aliases.map(rustString).join(', ')}] },`,
  );
...
function renderTypeScript(registry, digest) {
  const families = registry.families.map(({ family, canonical_unit, quantity_kind, aliases }) => ({
    family,
...
function renderMarkdown(registry, digest) {
  const familyRows = registry.families.map((family) =>
    `| \`${family.family}\` | \`${family.quantity_kind}\` | \`${family.canonical_unit}\` | ${family.aliases.map((alias) => `\`${alias}\``).join(', ')} |`,
  );
...
- Population: ${registry.families.length} families, ${registry.families.reduce((sum, family) => sum + family.aliases.length, 0)} aliases, ${registry.unit_tokens.length} unit tokens, ${registry.rules.length} conversion rules\n\
\n\
## Families\n\
\n\
