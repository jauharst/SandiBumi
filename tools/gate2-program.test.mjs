...
test('the_live_gate_two_progress_receipt_accounts_for_every_handled_row_once', () => {
  ...
  assert.equal(program.completed_requirements.length, 144);
  assert.equal(program.blocked_requirements.length, 62);
  assert.equal(handled.size, 206, 'completed and blocked receipts must not overlap');
  assert.equal(program.gate2_requirement_count - handled.size, 16);
