...
test('the_live_gate_two_progress_receipt_accounts_for_every_handled_row_once', () => {
  ...
  assert.equal(program.completed_requirements.length, 124);
  assert.equal(program.blocked_requirements.length, 46);
  assert.equal(handled.size, 170, 'completed and blocked receipts must not overlap');
  assert.equal(program.gate2_requirement_count - handled.size, 52);
  ...
  assert.ok(program.blocked_requirements.includes('SB-POR-011'));
  assert.ok(program.blocked_requirements.includes('SB-POR-021'));
  assert.ok(program.blocked_requirements.includes('SB-POR-023'));
});
