...
test('the_live_inventory_owns_31_warnings_and_39_ignored_tests_without_counting_them_as_passed', () => {
  // CHARACTERIZATION — the live compiler/test inventory is 31 warnings and 39 ignored tests
  ...
  // asserts nothing, so the green gate must never wait on its multi-well chain)
  ...
  assert.equal(ignoredInventory.expected_ignored_test_count, 39);
