...
test('the_live_inventory_owns_31_warnings_and_45_ignored_tests_without_counting_them_as_passed', () => {
  // CHARACTERIZATION — the live compiler/test inventory is 31 warnings and 45 ignored tests
  ...
  // where a module run's time actually goes; the 42nd measures the same question for the Field
  ...
  // back-fill run from inside a read - after six hypotheses had been ruled out; the 45th reads
  // every well's curves three ways - one lock around a serial loop, one lock per well across
  // eight threads, and one pooled reader per well - which is how SB-CORE-032 got measured for the
  // interactive plot path and how the lock-granularity hypothesis got refuted)
  ...
  assert.equal(ignoredInventory.expected_ignored_test_count, 45);
