...
<aside class="launch-aside" aria-hidden="true">
  ...
  <div class="launch-art launch-art--flip">
    <svg viewBox="0 0 320 240" preserveAspectRatio="xMidYMid slice">
      <rect width="320" height="240" fill="url(#bg-strata)"/>
      <g fill="none" stroke-linecap="round">
        <path d="M-10 62 Q 80 30 160 58 T 330 44" stroke="#c67139" stroke-width="26" opacity=".85"/>
...
const CSS = `
  ...
  * { box-sizing: border-box; }
  ...
  /* ── the launch screen, echoed in the wide-screen margin ──
     On a wide monitor the 900px reading column leaves a bare cream field to its
     right. The margin carries the application's own launch-screen identity, and
     it fills the field the way the launch screen fills its window: strata waves
     at the top, the mark and wordmark centered, mirrored waves and the edition
     line at the bottom — a full-height column, not a floating island. Decorative
     only (aria-hidden, no pointer events), fixed so it never enters the reading
     flow, and shown only from 1600px up.

     The column alone is not the fix: a left-anchored reading column with a strip
     pinned to the far right leaves the void BETWEEN them, which is the shape that
     reads as broken. So above 1600px the main column also RESERVES the strip's
     width and centres the measure in what is left — 1900px gives 1900 − 256 rail
     − 264 strip = 1380 for a 900px measure, so the leftover falls as two equal
     margins instead of one lopsided field. */
  .launch-aside {
    display: none; position: fixed; right: 0; top: 0; bottom: 0; width: 264px;
    pointer-events: none; background: var(--color-card);
    border-left: 1px solid rgba(32, 30, 29, .16);
    flex-direction: column; overflow: hidden;
  }
  @media (min-width: 1600px) {
    .launch-aside { display: flex; }
    .main { padding-right: 284px; }
    .main-inner { margin-inline: auto; }
  }
  .launch-art { flex: 0 0 auto; }
  .launch-art svg { display: block; width: 100%; height: auto; }
  .launch-art--flip svg { transform: scaleY(-1); }
  .launch-body { margin: auto 0; padding: 24px 18px; text-align: center; }
  .launch-body img { width: 52px; height: 52px; margin: 0 auto 10px; display: block; }
  .launch-word {
    font: 700 22px/1.2 var(--font-body); letter-spacing: -.02em;
    color: var(--color-accent-700);
  }
  .launch-desc { font: 400 12px/1.5 var(--font-body); color: var(--color-neutral-800); margin-top: 4px; }
  .launch-foot {
    flex: 0 0 auto; text-align: center;
    font: 400 10.5px/1.6 var(--font-body); color: var(--color-neutral-700);
    border-top: 1px solid var(--row-line); padding: 10px 18px 14px;
  }
`;
